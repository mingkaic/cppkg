...
TEST(CONTAIN, MustGet)
{
	...
	std::string expect_msg1 = sprintf(ohnos, 7);
	std::string expect_msg2 = sprintf(ohnos, 8);
	std::string expect_msg3 = sprintf(ohnos, 9);
	std::string expect_msg4 = sprintf(ohnos, 0);
	...
	EXPECT_CALL(logger, log(FATAL, expect_msg4, _)).Times(1).WillOnce(Throw(TestException(expect_msg4)));
