...
using MsgDiffT = std::vector<DiffArrT<cisab::types::StringsT>>;
...
static void process_lines (cisab::types::StringsT& lines, std::istream& str,
...
static void safe_diff_msg_helper (MsgDiffT& diffs,
	const cisab::types::StringsT& expect,
	...
	size_t batch_limit)
{
	...
	auto goit = got.begin();
	cisab::types::StringsT exbatch(exit, exit + std::min(batch_limit, nexpect));
	cisab::types::StringsT gobatch(goit, goit + std::min(batch_limit, ngot));
	...
	safe_diff_msg_helper(diffs,
		cisab::types::StringsT(exit + eoffset, expect.end()),
		cisab::types::StringsT(goit + goffset, got.end()), batch_limit);
}
...
std::string diff_lines (
	...
	bool ignore_empty_lines, bool trim_spaces)
{
	cisab::types::StringsT exlines;
	cisab::types::StringsT golines;
