...
#include "cppkg/diff/msg.h"


using namespace cppkg::diff;
...
{
	EXPECT_EQ(0, myers_diff_min_edit<std::string>(
		"ABCABBA", "ABCABBA"));
	EXPECT_EQ(5, myers_diff_min_edit<std::string>(
		"ABCABBA", "CBABAC"));
	EXPECT_EQ(13, myers_diff_min_edit<std::string>(
		"ABCDEFG", "HIJKLM"));
}
...
TEST(DIFF, MyersDiff)
{
	...
	auto same = myers_diff<std::string>(s, "ABCABBA");
	for (size_t i = 0, n = same.size(); i < n; ++i)
	{
		EXPECT_EQ(s[i], same[i].val_);
		EXPECT_EQ(i, same[i].orig_);
		EXPECT_EQ(i, same[i].updated_);
		EXPECT_EQ(EQ, same[i].action_);
	}

	std::string similar = "CBABAC";
	auto notsame = myers_diff<std::string>(s, similar);
	cisab::types::StringsT expectations = {
		"- 0\t \tA\n",
		"- 1\t \tB\n",
		"  2\t0\tC\n",
		"+  \t1\tB\n",
	...
	{
		std::stringstream ss;
		diff_line_format(ss, notsame[i].val_,
	...
	auto different = myers_diff<std::string>(s, other);
	size_t n = s.size();
	for (size_t i = 0; i < n; ++i)
	{
		...
		EXPECT_EQ(DEL, different[i].action_);
	}
	for (size_t i = 0, m = other.size(); i < m; ++i)
	{
		...
		EXPECT_EQ(ADD, different[n + i].action_);
	}
}
...
{
	auto same_prefix = safe_diff_msg({
		"advise",
		"apathetic",
		"disappear",
		"greasy",
	...
	auto same_prefix2 = safe_diff_msg({
		"advise",
		"apathetic",
		"disappear",
		"greasy",
...
{
	auto same_postfix = safe_diff_msg({
		"advise",
		"apathetic",
		"disappear",
		"greasy",
...
{
	auto small_overlap = safe_diff_msg({
		"advise",
		"apathetic",
		"disappear",
		"greasy",
...
{
	auto bigger_overlap = safe_diff_msg({
		"advise",
		"apathetic",
		"disappear",
		"greasy",
...
TEST(DIFF, DiffLines)
{
	...
	auto diff = diff_lines(left, right, false, false); // compare as is
	EXPECT_STREQ(
		"- 0\t \t advise  \n"
		"+  \t0\tadvise\n"
		"  1\t1\t  apathetic\n"
	...
	auto diff2 = diff_lines(left2, right2, true, false); // ignore empty lines
	EXPECT_STREQ(
		"- 0\t \t advise  \n"
		"+  \t0\tadvise\n"
		"  1\t1\t  apathetic\n"
	...
	auto diff3 = diff_lines(left3, right3, false, true); // trim space
	EXPECT_STREQ(
		"  0\t0\tadvise\n"
		"  1\t1\tapathetic\n"
		"- 2\t \t\n"
	...
	auto diff4 = diff_lines(left4, right4, true, true); // full cleanup
