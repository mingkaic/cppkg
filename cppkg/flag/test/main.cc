...
struct TestLogger : public cisab::logs::iLogger
{
	...
	void log (size_t msg_level, const std::string& msg,
		const cisab::logs::SrcLocT& location = cisab::logs::SrcLocT::current()) override
	{
		switch (msg_level)
		{
			case cisab::logs::WARN:
				warn(msg);
				break;
			case cisab::logs::ERROR:
				error(msg);
				break;
			case cisab::logs::FATAL:
				fatal(msg);
				break;
			default:
				std::stringstream ss;
	...
	void log (const std::string& msg_level, const std::string& msg,
		const cisab::logs::SrcLocT& location = cisab::logs::SrcLocT::current()) override
	{
		log(cisab::logs::enum_log(msg_level), msg);
	}
...
{
	cisab::logs::set_logger(std::static_pointer_cast<cisab::logs::iLogger>(tlogger));
