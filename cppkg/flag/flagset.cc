...
FlagSet::FlagSet (const char* prog) :
	flags_(cisab::fmts::sprintf("%s flags", prog)),
	args_(cisab::fmts::sprintf("%s arguments", prog)) {}
...
bool FlagSet::parse(int argc, const char** argv, opt::variables_map& vars)
{
	...
	try
	{
		...
		if (vars.count("help"))
		{
			cisab::logs::warn(make_usage_string(boost::filesystem::path(
