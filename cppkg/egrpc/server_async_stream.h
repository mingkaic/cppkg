...
#ifndef CPPKG_EGRPC_SERVER_ASYNC_STREAM_H
#define CPPKG_EGRPC_SERVER_ASYNC_STREAM_H
...
struct AsyncServerStreamCall final : public iServerCall
{
	...
	AsyncServerStreamCall (std::shared_ptr<cisab::logs::iLogger> logger,
		...
		req_call_(req_call), init_call_(init_call), write_call_(write_call)
	{
		...
		logger_->log(cisab::logs::info_level, cisab::fmts::sprintf("rpc %p created", this));
	}
	...
	void serve (void) override
	{
		...
		case STARTUP:
		{
			...
			logger_->log(cisab::logs::info_level, cisab::fmts::sprintf("rpc %p initializing", this));
		...
		case PROCESS:
		{
			...
			{
				RES reply;
				logger_->log(cisab::logs::info_level, cisab::fmts::sprintf("rpc %p writing", this));
				bool wrote = false;
			...
			break;
		case FINISH:
			logger_->log(cisab::logs::info_level, cisab::fmts::sprintf("rpc %p completed", this));
			shutdown();
		}
	}
...
#endif // CPPKG_EGRPC_SERVER_ASYNC_STREAM_H
